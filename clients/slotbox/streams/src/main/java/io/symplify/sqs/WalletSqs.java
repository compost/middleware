...
public class WalletSqs {
  ...
  public static class DepositSqs extends WalletSqs{}
  public static class WithdrawalSqs extends WalletSqs{}
  ...
  public static Optional<WalletSqs> transform(WalletKafka kafka) {
    var type = kafka.transaction_type_id.orElse("").toLowerCase();
  ...
  private static Optional<WalletSqs> transformWithdraw(WalletKafka kafka) {
    var status = kafka.transaction_status_id.orElse("").toLowerCase();
  ...
  public static class SuccessWithdrawal extends WalletSqs {
    ...
    public void with(WalletKafka kafka) {
      ...
      this.Transaction_Type = kafka.transaction_type_id;
      this.last_withdrawal = kafka.amount;
      this.last_withdrawal_date = kafka.transaction_datetime.map(Transformer::truncateOrKeep);
    }
  }
  ...
  public static class SuccessDeposit extends WalletSqs {
    ...
    public void with(WalletKafka kafka) {
      ...
      this.last_deposit = kafka.amount;
      this.last_deposit_date = kafka.transaction_datetime.map(Transformer::truncateOrKeep);
    }
  }
}
