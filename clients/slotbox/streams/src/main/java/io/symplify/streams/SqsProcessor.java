...
public class SqsProcessor implements Processor<String, byte[], Void, Void> {
  ...
  private Optional<PlayerStore> processMesssage(RecordMetadata metadata, Record<String, byte[]> record) {
    try {
      switch (metadata.topic()) {
        case Configuration.Topic.REPARTITIONED_PLAYERS:
          return onTopicPlayers(record);
        case Configuration.Topic.REPARTITIONED_PLAYER_CONSENT:
          return onTopicPlayersConsent(record);
        case Configuration.Topic.REPARTITIONED_WALLET:
          return onTopicWallet(record);
        case Configuration.Topic.REPARTITIONED_PLAYER_STATUS:
  ...
  private Optional<PlayerStore> onTopicPlayerStatus(Record<String, byte[]> record)
    ...
    final Optional<String> previousValue = playerInStore.flatMap(v -> v.status).flatMap(v -> v.is_blocked);
    final Optional<PlayerStore> newPlayer = playerInStore.or(() -> Optional.ofNullable(new PlayerStore()))
        .flatMap(ps -> ps.with(kafka));
    final Optional<String> newValue = newPlayer.flatMap(v -> v.status)
        .flatMap(v -> v.is_blocked);
    if (previousValue != newValue) {
      PlayerBlockedSqs.transform(kafka)
          .ifPresent(
              sqs -> sender.sendFull(kafka.brand_id.get(), kafka.player_id.get(), sqs.type, sqs.mappingSelector,
                  sqs));
    }
    return newPlayer;
  }
  ...
  private Optional<PlayerStore> onTopicPlayersConsent(Record<String, byte[]> record) throws IOException {
    final PlayerConsentKafka kafka = mapper.readValue(record.value(),
        PlayerConsentKafka.class);

    return PlayerConsentSqs.transform(kafka, Type.USER_CONSENT_UPDATE, Selector.PLAYER_CONSENT)
        .flatMap(sqs -> {
          final Optional<PlayerStore> playerInStore = Optional.ofNullable(store.get(record.key()));
          final Optional<PlayerStore> newPlayer = playerInStore.or(() -> Optional.ofNullable(new PlayerStore()))
              .flatMap(ps -> ps.with(kafka));
          newPlayer
              .ifPresent(s -> sender.sendFull(sqs.brand_id.get(), sqs.player_id.get(), Type.USER_CONSENT_UPDATE,
                  Selector.PLAYER_CONSENT,
                  sqs));
          return newPlayer;
        });
