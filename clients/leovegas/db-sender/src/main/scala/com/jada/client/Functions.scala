...
class Functions {
  ...
  val clientId = getenv("CLIENT_ID", "9").toInt
  val topic = getenv("TOPIC", "players-history")
  ...
  def runProducePlayers(
    ...
    val df = spark.read
      ...
      .option(
        "query",
        s"""
          SELECT p.player_id as player_id, TO_VARCHAR(p.country_id) as country_id 
          FROM dbo.dim_players p INNER JOIN dbo.dim_brands b ON p.brand_id = b.brand_id
          WHERE b.client_id = ${clientId} 
          |""".stripMargin
      )
      .options(sfOptions).load()

    import spark.implicits._
    import functions._
    val newDf = df.select(
      df.col("player_id").as("key"),
      to_json(struct($"player_id", $"country_id")).as(
        "value"
      )
    )
    newDf.show(1)
