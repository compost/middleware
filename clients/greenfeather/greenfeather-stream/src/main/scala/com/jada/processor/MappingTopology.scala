...
class MappingTopology @Inject() (
  ...
  def buildTopology(): Topology = {
    ...
    builder
      .stream[String, PlayerBatch](
        config.topicPlayersBatch
      )(Consumed.`with`(stringSerde, CirceSerdes.serde[PlayerBatch]))
      .filter((_, v) =>
        v.player_id.isDefined && v.brand_id.exists(b => brandIds.contains(b))
      )
      .selectKey((_, v) => generateKey(v.brand_id, v.player_id))
      .to(config.topicPlayersBatchRepartitioned)(
        Produced.`with`(stringSerde, CirceSerdes.serde[PlayerBatch])
      )
    ...
    builder
      .stream[String, BonusTransaction](
        config.topicBonusTransaction
      )(Consumed.`with`(stringSerde, CirceSerdes.serde[BonusTransaction]))
      .filter((_, v) =>
        v.player_id.isDefined && v.brand_id.exists(b => brandIds.contains(b))
      )
      .selectKey((_, v) => generateKey(v.brand_id, v.player_id))
      .to(config.topicBonusTransactionRepartitioned)(
        Produced.`with`(stringSerde, CirceSerdes.serde[BonusTransaction])
      )

    builder
      .stream[String, UserConsentUpdate](
        config.topicUserConsentUpdate
      )(Consumed.`with`(stringSerde, CirceSerdes.serde[UserConsentUpdate]))
      .filter((_, v) =>
        v.player_id.isDefined && v.brand_id.exists(b => brandIds.contains(b))
      )
      .selectKey((_, v) => generateKey(v.brand_id, v.player_id))
      .to(config.topicUserConsentUpdateRepartitioned)(
        Produced.`with`(stringSerde, CirceSerdes.serde[UserConsentUpdate])
      )

    builder
      .stream[String, ActionTrigger](
        config.topicActionTrigger
      )(Consumed.`with`(stringSerde, CirceSerdes.serde[ActionTrigger]))
      .filter((_, v) =>
        v.player_id.isDefined && v.brand_id.exists(b => brandIds.contains(b))
      )
      .selectKey((_, v) => generateKey(v.brand_id, v.player_id))
      .to(config.topicActionTriggerRepartitioned)(
