...
class MappingTopology @Inject() (
  ...
  def buildTopology(): Topology = {
    ...
    val storePlayerSelfStoreName = "player-self-store"

    val stringSerde: Serde[String] = Serdes.String
    val playerStoreSerde = CirceSerdes.serde[PlayerStore]
    implicit val consumed = Consumed.`with`(stringSerde,playerStoreSerde)
    ...
    if (config.inMemory) {
      ...
      builder.addStateStore(
        Stores.keyValueStoreBuilder(
          Stores.inMemoryKeyValueStore(storePlayerSelfStoreName),
          stringSerde,
          playerStoreSerde
        )
      )
      ...
      builder.addStateStore(
        Stores.keyValueStoreBuilder(
          Stores.persistentKeyValueStore(storePlayerSelfStoreName),
          stringSerde,
          playerStoreSerde
        )
      )
    ...
    val kstream: KStream[String, PlayerStore] =
      builder.stream[String, PlayerStore](config.appTopics)(consumed)
