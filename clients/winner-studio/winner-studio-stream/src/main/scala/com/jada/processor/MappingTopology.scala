...
import com.jada.models.{Action, Login, Player, PlayerExtended, PlayerStatus, PlayerStore, UserConsentUpdate, Wagering, Wallet}
import io.circe._
...
class MappingTopology @Inject() (
  ...
  def buildTopology(): Topology = {
    ...
    val balanceStoreName = "balance-store"
    ...
    builder
      .stream[String, UserConsentUpdate](
        config.topicUserConsentUpdateWinnerStudio
      )(Consumed.`with`(stringSerde, CirceSerdes.serde[UserConsentUpdate]))
      .filter((_, v) =>
        v.player_id.isDefined && v.brand_id.exists(b => brandIds.contains(b))
      )
      .selectKey((_, v) => generateKey(v.brand_id, v.player_id))
      .to(config.topicUserConsentUpdateWinnerStudioRepartitioned)(
