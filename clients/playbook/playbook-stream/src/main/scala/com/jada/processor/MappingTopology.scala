...
import com.jada.DefaultTopology
import com.jada.BalanceTransformer
import com.jada.BalanceTopology
...
class MappingTopology @Inject() (
  ...
  @Produces
  @BalanceTopology
  def buildBalanceTopology(): Option[Topology] = {
    if (config.balanceTopologyEnabled) {

      val storeBalanceName = "balance-store"
      val stringSerde: Serde[String] = Serdes.String
      val walletSerde = CirceSerdes.serde[Wallet]
      implicit val consumed = Consumed.`with`(stringSerde, walletSerde)
      val builder = new StreamsBuilder
      builder.addStateStore(
        Stores.keyValueStoreBuilder(
          Stores.persistentKeyValueStore(storeBalanceName),
          stringSerde,
          stringSerde
        )
      )
      ...
      val kstream: KStream[String, Wallet] =
        builder.stream[String, Wallet](Set(config.topicWallets))(consumed)
  ...
  @Produces
  @DefaultTopology
  def buildDefaultTopology(): Option[Topology] = {
    if (config.defaultTopologyEnabled) {
      ...
      implicit val consumedCurrency =
        Consumed.`with`(stringSerde, currencySerde)
      ...
      builder
        ...
        .filter((_, v) =>
          v.currency_id.isDefined && v.brand_id.exists(b =>
            brandIds.contains(b)
          )
        )
