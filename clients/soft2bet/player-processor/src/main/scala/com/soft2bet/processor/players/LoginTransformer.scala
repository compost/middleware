...
class LoginTransformer(
    ...
    loginStore: String
) extends Transformer[String, Login, KeyValue[String, Login]]
    with Punctuator {
  ...
  override def transform(
      key: String,
      value: Login
  ): KeyValue[String, Login] = {
    val v = if (value.login_success.getOrElse("") == "1") {
      val toStored = Option(store.get(key)) match {
        case None => value
        case Some(old) =>
          old.copy(
            login_datetime = value.login_datetime
          )
      }
      store.put(key, toStored)
      toStored
    } else {
      value
    }
  ...
  def writeFile(
    ...
    val tmp = Files.createTempFile(s"ldc-login-${prefix}", ".json")
    ...
    try {
      ...
      while (login.hasNext()) {
        ...
        val loginJson = printer.print(
          LoginJSON.loginSQSEncoder.apply(
            LoginJSON(keyAndValue.value)
          )
        )
        val line =
